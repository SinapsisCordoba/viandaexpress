    ...
    <body>
        ...
        <div class="container" style="margin-top:20px; width: 98%;">
            <table class="table table-striped" id="pedidos">
                ...
                <tbody>
                    <?php
                        $sql = "SELECT `id`, `sucursal`, `total`, `cliente_nombre`, `cliente_telefono`, `cliente_direccion`, `cliente_email`, `envio`, `pedido`, `cantidad_menus`, `hora`, `estado`, TIME(`marca_temporal`) AS 'marca_temporal' FROM `pedido` WHERE DATE(`marca_temporal`) = CURDATE() ORDER BY `marca_temporal` DESC ";
                        $result = connectarDB($sql);
